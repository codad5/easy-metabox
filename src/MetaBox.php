...
namespace Codad5\EasyMetabox;

use Codad5\EasyMetabox\helpers\InputValidator;
use Codad5\EasyMetabox\helpers\ViewLoader;
use WP_Post;
...
class MetaBox
{
    ...
    {
        return "codad5-easy-metabox-quick-edit-{$this->nonce}";
    }

    function get_fetch_data_nonce(): string
    {
        return "codad5-easy-metabox-fetch-data-{$this->nonce}";
    }
    ...
    function setup_actions(): void
    {
        ...
        add_action( "wp_ajax_cem_{$this->id}_fetch_post_by_id", [$this, 'handle_incoming_ajax']);
        add_action( "wp_ajax_nopriv_cem_{$this->id}_fetch_post_by_id", [$this, 'handle_incoming_ajax'] );
    ...
    function show_quick_edit_field($column_name, $post_type): void
    {
...
//        var_dump($field, $column_name);
    ...
    {
        $fields = array_filter($this->fields, fn($field) => $field['allow_quick_edit'] === true);
        $screen = \get_current_screen();

        // Only add to Movies admin list
        if (!$screen || $screen->post_type !== $this->screen || $screen->base !== 'edit') {
            return;
        }


        wp_enqueue_script('codad5-easy-metabox-movie-quick-edit',plugin_dir_url(__FILE__) . '/../../assets/scripts/movie-quick-edit.js',   ['jquery', 'inline-edit-post'], rand(0,1000), true);
        wp_localize_script("codad5-easy-metabox-movie-quick-edit", 'codad5EasyMetaboxQuickEdit', [
           'quick_edit_fields' => array_values(array_map(fn($f) => $f['id'], $fields)),
           'meta_box_id' => $this->id,
    ...
    // enqueue scripts to the admin for post type codad5-easy-metabox-wp_widget
    function enqueue_scripts($path, $handle = 'codad5-easy-metabox-wp_admin_script', $deps = [], $ver = false, $in_footer = true): void
    {
    ...
    function enqueue_styles($path, $handle = 'codad5-easy-metabox-wp_admin_style', $deps = [], $ver = false, $media = 'all'): void
    {
    ...
    private function generate_wp_media_html(string $id, array $data): string
    {
        ...
        $attributes = isset($data['attributes']) && is_array($data['attributes']) ? $data['attributes'] : [];
        $attributes['class'] = esc_attr(($attributes['class'] ?? '') . ' codad5-easy-wpmetabox-input');
        $required = !empty($attributes['required']);
        ...
        ViewLoader::load('wp-media-field', array(
            'id' => $id,
            'label' => $label,
        ...
        if (!empty($has_error)) {
            ...
            if ($this->show_admin_error) {
                add_filter('redirect_post_location', function ($location) use ($has_error) {
                    ...
                    set_transient('codad5-easy-metabox-wp_error', urlencode("Error saving fields: \" {$error_message} \" "), 30);
                    return add_query_arg('codad5-easy-metabox-wp_error', urlencode("Error saving fields: {$error_message}"), $location);
                });
            }
...
//        }, $check_field);
    ...
    {
        $error = get_transient('codad5-easy-metabox-wp_error');
        if ($error) {
            $error = urldecode($error);
            echo "<div class='notice notice-error is-dismissible'><p>{$error}</p></div>";
            delete_transient('codad5-easy-metabox-wp_error');
        }
    }
    ...
    function handle_incoming_ajax(): void
    {
