    ...
    <div class="relative overflow-hidden h-24">
        <div class="flex w-full h-full animate-marquee whitespace-nowrap">
            ...
            <?php foreach ($distribution_partners as ['title' => $title, 'description' => $description, 'logo' => $logo]) : ?>
                <div class="h-full p-4 lg:py-5 lg:px-7 grid place-items-center shrink-0 lg:shrink-0">
                    <img src="<?php echo esc_url(CeanWP_Functions::get_common_icon_url($logo)); ?>" class="w-12 lg:w-28 h-6 lg:h-10 object-contain" alt="<?php echo $title; ?>" />
                </div>
            <?php endforeach; ?>
